                            ...
                            float omega)
{
   ...
   for(size_t tuvw=0; tuvw<(size_t)image->nt*image->nu*image->nv*image->nw; ++tuvw){
      ...
      for(int i=0; i<image->ny*image->nz; i++)
      {
         ...
         restoreLine<DTYPE>(start,end,increment,currentCoeffPtr,imageValues);
      }
      delete[] coeffValues;
      delete[] imageValues;
      coeffValues=NULL;
      ...
      for(i=0; i<image->ny*image->nz; ++i)
      {
         ...
         restoreLine<DTYPE>(start,end,increment,currentImagePtr,coeffValues);
      }
      delete[] coeffValues;
      coeffValues=NULL;
                       ...
                       float lambda)
{
   ...
   if (flag_debug){
      nifti_set_filenames(img, "/tmp/before_smoothing.nii.gz", 0, 0);
   ...
   if (flag_debug){
      nifti_set_filenames(img, "/tmp/after_smoothing.nii.gz", 0, 0);
      nifti_image_write(img);
   }
